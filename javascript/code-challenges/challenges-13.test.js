...
const findHappiness = (arr) => {
  ...
  let regexOne = /^:\)/g;
  let regexTwo = /:\)/g;
  let match = arr.filter(element => {
    if ((regexOne.test(element)) || (regexTwo.test(element)))
      return element;
  });
  return match;
...
const allHappy = (arr) => {
  ...
  let newArr = arr.filter(element => {
    if ((regexTwo.test(element)) || (regexOne.test(element))) {
      return element;
    }
  });
